...
async def cmd_diag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Диагностика подключения к iiko"""
    if not check_access(update.effective_user.id):
        return

    msg = await update.message.reply_text("🔍 Запускаю диагностику iiko...")
    try:
        result = await iiko.run_diagnostics()
        await msg.edit_text(result)
    except Exception as e:
        await msg.edit_text(f"⚠️ Ошибка диагностики: {e}")
