...
class IikoClient:
    ...
    async def _collect_all_orders(self, date_from: str, date_to: str) -> list:
        ...
        # Способ 2: Заказы зала (table orders) по дате
        try:
            methods_tried.append("order/by_table")
            tg_ids = await self.get_terminal_group_ids()
            for tg_id in tg_ids:
                data = await self._safe_post("/api/1/order/by_table", {
                    "organizationIds": [org_id],
                    "terminalGroupIds": [tg_id],
                    "dateFrom": f"{date_from} 00:00:00.000",
                    "dateTo": f"{date_to} 23:59:59.999",
                ...
                if data:
                    for org in data.get("ordersByOrganizations", []):
                        ...
                        all_orders.extend(orders)
                        if orders:
                            methods_success.append(f"table orders ({tg_id[:8]}): {len(orders)} заказов")
        except Exception as e:
            logger.warning(f"order/by_table не сработал: {e}")
        ...
        try:
            ...
            if tg_ids:
                endpoints.append(("order/by_table", "/api/1/order/by_table", {
                    "organizationIds": [org_id],
                    "terminalGroupIds": tg_ids[:1],
                    "dateFrom": f"{yesterday} 00:00:00.000",
                    "dateTo": f"{today} 23:59:59.999",
                    "statuses": ["Closed", "Bill"]
                }))
        except Exception:
            pass
